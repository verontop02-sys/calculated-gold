...
/** Модерация клиентов fintech-биржи Reaktivo Invest: KYC, статус, ручное пополнение. */
export function FintechAdminPage({ toast }) {
  ...
  return (
    <div className="cg-page">
      ...
      <aside className="cg-page__side cg-stagger">
        {selectedId ? (
          <ClientDetailPanel clientId={selectedId} toast={toast} onChanged={() => { load(); }} onClose={() => setSelectedId(null)} />
        ) : (
          <div className="fea-card fea-card--accent">
            <div className="fea-side-label">Клиенты биржи</div>
            <div className="fea-side-big mono-nums">{total}</div>
            <div className="fea-side-sub">Выберите клиента в списке, чтобы проверить документы и управлять балансом</div>
          </div>
        )}
      </aside>
...
function StatusBadge({ status }) {
  return <span className={`fea-badge fea-badge--${status}`}>{STATUS_LABEL[status] || status}</span>;
}

function ClientDetailPanel({ clientId, toast, onChanged, onClose }) {
...
.fea-badge { font-size: 0.68rem; font-weight: 700; padding: 3px 9px; border-radius: 999px; display: inline-block; }
